...
static l_table lut[TOTAL_NODES+3] = {
...
static void send_packet(packet_t tx_packet){
	...
	// if LEDS_RED use reverse lookup table for reverse direction
	if(tx_packet.message == LEDS_RED)
	{
		for(i = 0; i < TOTAL_NODES ; i++)
		{
			if(linkaddr_cmp(&tx_packet.dest, &lut[(node_id + 3) - 1].dest[i]))
			{
				packetbuf_copyfrom(&tx_packet, sizeof(packet_t));
				unicast_send(&unicast, &lut[(node_id+3) - 1].next_hop[i]);
				break;
			}
		}
	}
	// for other colors use normal direction
		...
		for(i = 0; i < TOTAL_NODES; i++) //TOTAL_NODES = 3
		{
